package com.mobiai.app.manifier;
...
public class CameraTextureView extends TextureView
{
        ...
        public void onImageAvailable( ImageReader reader )
        {
            ...
            try
            {
                AsyncTask.execute( () -> {
                    ...
                    Log.d("tuan", "mImageByteArray 2:  " + mImageByteArray);
                    ByteBuffer buffer = image.getPlanes()[ 0 ].getBuffer();
                    mImageByteArray = new byte[ buffer.capacity() ];

                    Log.d("tuan", "mImageByteArray 3:  " + mImageByteArray);
                    mImageOrientation = getOrientation();
                    buffer.get( mImageByteArray );

                    Log.d("tuan", "mImageByteArray 4:  " + mImageByteArray);
                    image.close();
                    ...
                    if ( !mDisplayPreview )
                    {
                        ...
                        Log.d("tuan", "mImageByteArray 5:  " + mImageByteArray);
                    }
                    else
                    {
    ...
    protected CameraCaptureSession.CaptureCallback mCaptureCallback = new CameraCaptureSession.CaptureCallback()
    {
        ...
        private void process( CaptureResult result )
        {
            switch ( mState )
            {
                ...
                {
                    Log.v( TAG, "state: STATE_WAITING_LOCK , result: " + result);


                       Integer i = result.get(CaptureResult.CONTROL_AF_MODE);
                       if (i == null) return;
                       final int afState = result.get( CaptureResult.CONTROL_AF_MODE );
                ...
                {
                    Integer i = result.get(CaptureResult.CONTROL_AE_STATE);
                    if (i == null) return;
    ...
    public void setFacing( int facing )
    {
        if ( facing == LENS_FACING_BACK || facing == LENS_FACING_FRONT ) {
            Log.d(TAG, "LENS_FACING_BACK" + LENS_FACING_BACK);
            Log.d(TAG, "LENS_FACING_FRONT" + LENS_FACING_FRONT);
            mFacing = facing;
...
//                mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, 12);
    ...
    public void setBrightness(int bright)
    {
        try
        {
            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, bright);
            mCaptureSession.setRepeatingRequest( mPreviewRequestBuilder.build(), mCaptureCallback, null );
        }
        catch ( NullPointerException | CameraAccessException e )
        {
    ...
    {
        Log.d("tuan", " getImageByteArray ----------> " + mImageByteArray);
    ...
    protected void setUpCameraOutputs( int width, int height )
    {
        ...
        try
        {
           ...
           mCameraCharacteristics = manager.getCameraCharacteristics( cameraId );
            ...
            Log.d(TAG, "support auto focus : " + isAfSupported);
